...
# Generate PDP data for humidity

pdp_data_hum <- partial(rf_day, pred.var = "hum", train = day_new, grid.resolution = 20)


# Convert the pdp_data to a data frame
pdp_df_hum <- as.data.frame(pdp_data_hum)

# Generate the plot
pdp_plot_hum <- ggplot(pdp_df_hum) +
  #geom_density(fill = "lightblue", alpha = 0.5) +  # Density plot
  geom_line(aes(x = hum, y = yhat), color = "black") +  # Line plot for PDP
  geom_rug(data = day_new, aes(x = hum), sides = "b") +
  theme_minimal() +
  labs(title = "Partial Dependency Plot for Humidity",
